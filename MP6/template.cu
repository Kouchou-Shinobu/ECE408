...
//@@ insert code here
__global__
void FloatToUchar(unsigned char* ucharImage, const float* floatImage, int height, int width, int imageChannels){
...
__global__
void RGBToGray(unsigned char* grayImage, const unsigned char* rgbImage, int height, int width, int imageChannels){
    int Col = threadIdx.x + blockIdx.x * blockDim.x;
    int Row = threadIdx.y + blockIdx.y * blockDim.y;
    if(Col < width && Row < height) {
...
__global__
void HistoEqualization(unsigned char* ucharImage, const float* cdf, int height, int width, int imageChannels){
